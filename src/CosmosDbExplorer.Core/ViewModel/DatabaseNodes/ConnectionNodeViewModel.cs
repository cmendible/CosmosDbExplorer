...
namespace CosmosDbExplorer.ViewModel
{
    public class ConnectionNodeViewModel : TreeViewItemViewModel, ICanRefreshNode
    {
        ...
        public ConnectionNodeViewModel(IDocumentDbService dbService, IEventAggregator messenger, IDialogService dialogService, ISettingsService settingsService) : base(null, messenger, true)
        {
                            ...
                            await _dialogService.ShowMessage("Are you sure that you want to delete this connection?", "Delete connection", null, null,
                                async confirm =>
                                {
                                    if (confirm)
                                    {
                                        ...
                                        MessengerInstance.GetEvent<RemoveConnectionMessage>().Publish(Connection);
                                    }
                                });
                        }
                        ));
